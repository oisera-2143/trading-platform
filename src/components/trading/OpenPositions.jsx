import React from 'react';
import { useTrading } from '../../context/TradingContext';
import { formatPrice, formatCurrency, formatPercent, getColorClass } from '../../utils/formatting';
import { X } from 'lucide-react';
import { Button } from '../common/Button';
import { Toast } from '../../utils/toast';

const OpenPositions = () => {
  const { account, closePosition } = useTrading();
  const [closing, setClosing] = React.useState(null);

  const handleClosePosition = async (positionId) => {
    setClosing(positionId);
    try {
      await closePosition(positionId);
      Toast.success('Position closed successfully');
    } catch (error) {
      Toast.error(error.message);
    } finally {
      setClosing(null);
    }
  };

  if (!account || account.positions.length === 0) {
    return (
      <div className="bg-secondary border border-tertiary rounded-lg p-6 text-center">
        <p className="text-neutral-dark">No open positions</p>
      </div>
    );
  }

  return (
    <div className="bg-secondary border border-tertiary rounded-lg overflow-hidden">
      <div className="p-4 sm:p-6 border-b border-tertiary">
        <h3 className="text-lg sm:text-xl font-semibold text-neutral">
          Open Positions ({account.positions.length})
        </h3>
      </div>

      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-tertiary">
            <tr>
              <th className="px-4 py-3 text-left text-neutral-dark">Asset</th>
              <th className="px-4 py-3 text-left text-neutral-dark">Type</th>
              <th className="px-4 py-3 text-right text-neutral-dark">Qty</th>
              <th className="px-4 py-3 text-right text-neutral-dark">Entry</th>
              <th className="px-4 py-3 text-right text-neutral-dark">Current</th>
              <th className="px-4 py-3 text-right text-neutral-dark">P/L</th>
              <th className="px-4 py-3 text-center text-neutral-dark">Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-tertiary">
            {account.positions.map((position) => (
              <tr key={position.id} className="hover:bg-tertiary transition-colors">
                <td className="px-4 py-3 font-semibold text-neutral">
                  {position.symbol}
                </td>
                <td className="px-4 py-3">
                  <span
                    className={`px-2 py-1 rounded text-xs font-semibold ${
                      position.type === 'buy'
                        ? 'bg-success bg-opacity-20 text-success'
                        : 'bg-danger bg-opacity-20 text-danger'
                    }`}
                  >
                    {position.type.toUpperCase()}
                  </span>
                </td>
                <td className="px-4 py-3 text-right font-mono text-neutral">
                  {position.quantity.toFixed(2)}
                </td>
                <td className="px-4 py-3 text-right font-mono text-neutral">
                  {formatPrice(position.entryPrice)}
                </td>
                <td className="px-4 py-3 text-right font-mono text-neutral">
                  {formatPrice(position.currentPrice)}
                </td>
                <td className={`px-4 py-3 text-right font-mono font-semibold ${getColorClass(position.pl)}`}>
                  <div>{formatCurrency(position.pl)}</div>
                  <div className="text-xs">{formatPercent(position.plPercent)}</div>
                </td>
                <td className="px-4 py-3 text-center">
                  <button
                    onClick={() => handleClosePosition(position.id)}
                    disabled={closing === position.id}
                    className="text-danger hover:text-opacity-80 transition-colors disabled:opacity-50"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
};

export default OpenPositions;
